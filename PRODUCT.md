...
## §42 Master Strip(总线母带链 / 缩混)—— 📐 设计 · 2026-06-29
...
export const DEFAULT_MASTER: MasterConfig = /* 全 on:false / 中性 */;
```
...
- 扩 `studio/ui/FxRack.tsx` 浮层:面包屑 `SENDS ▸ … ▸ OUT` 的 `OUT` 段扩成 **MASTER** 区(内容多,建议独立 tab)。EQ 三旋钮 + 宽度(width / monoBelow / air)+ 饱和(drive/character/mix)+ **压缩器子面板**(旋钮列 + **动态 GR 表** + Mix/SC/Auto-Rel 开关 + lookahead)+ limiter(ceiling/target LUFS)。
- 顶部加 **预设 chip 行**(§42.5)+ 一个 **LUFS/真峰小表**(读 `getMeters()`)。
- 沿用 MixerStrip 旋钮画法(竖拖 / 双击复位)+ `.metro-pop` 浮层范式 + 点外/Esc 关闭。

### §42.9 落地顺序(doc-first / 每步 typecheck)

① 本节。② `contracts`:`MasterConfig` + `DEFAULT_MASTER` 进 `FxConfig`(默认全旁路,跑通持久化/undo 零音色变)。③ `masterChain.ts`:`makeMasterStrip()` 接口骨架 + EQ/宽度/饱和(memoryless 三件,原生节点,立即可用零风险)。④ 压缩器 AudioWorklet(路线 A):实时跑通 envelope/SC HPF/lookahead/并行/GR 回传。⑤ `studioEngine.init` 接入 strip(master→strip→XY)+ `setFx` 推 `master` + `getMeters` 暴露。⑥ `exportSong` 接同一工厂 + `ready()`(离线 worklet 注册)。⑦ `FxRack` MASTER 区 UI + GR/LUFS 表。⑧ 预设系统 `masterPresets.ts` + 响度对齐 trim(需 LUFS:先 RMS 近似,后接 libebur128/Faust)。⑨〔路线 B〕Faust→WASM MasterDSP 换内核,接口不变。⑩ review:真机——套预设整曲音色变而响度不跳、压缩器走带多圈不抽吸(对照 §17)、GR/LUFS 表准、宽度过 mono 不塌、停播无残留、undo/redo、刷新还在、导出听感一致。
