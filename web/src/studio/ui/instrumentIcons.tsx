// 乐器图标库 —— 从 Iconify 内联(MDI + game-icons,均 currentColor 实心字形),零运行时依赖、离线可用。
// key 存进 Instrument.icon(默认 'wav')。要加新图标:从 api.iconify.design 取该图标 body 贴进这张表即可。
import type { CSSProperties } from 'react';

export interface IconDef { vb: string; body: string; label: string }

export const DEFAULT_ICON = 'wav';

export const INSTRUMENT_ICONS: Record<string, IconDef> = {
  "wav": {
    "vb": "0 0 24 24",
    "body": "<path fill=\"currentColor\" d=\"m22 12l-2 1l-1 1l-1-1l-1 3l-1-3l-1 8l-1-8l-1 2l-1-2l-1 4l-1-4l-1 9l-1-9l-1 6l-1-6l-1 1l-1-1l-2-1l2-1l1-1l1 1l1-6l1 6l1-9l1 9l1-4l1 4l1-2l1 2l1-8l1 8l1-3l1 3l1-1l1 1z\"/>",
    "label": "Waveform"
  },
  "note": {
    "vb": "0 0 24 24",
    "body": "<path fill=\"currentColor\" d=\"M12 3v10.55c-.59-.34-1.27-.55-2-.55c-2.21 0-4 1.79-4 4s1.79 4 4 4s4-1.79 4-4V7h4V3z\"/>",
    "label": "Note"
  },
  "drums": {
    "vb": "0 0 512 512",
    "body": "<path fill=\"currentColor\" d=\"m111 58.3l-87.37.4l-.61 8.3L192.4 92.6l1.8-8.1zm310.8 18.8l-.3 29.7l5-.8l4.9.8l-.3-29.7zM96.33 92.8l-1.81 13l-33.17 26.4l1.84 115.6l6.16-40.4l9.55-2.3h.28l-1.03-65l31.95-25.4l2.7-19.4zm330.17 25.9l-66.6 10.4l.6 8.3h132l.6-8.3zm-66 33.3l-.6 8.3l66.6 10.4l66.6-10.4l-.6-8.3zm60.3 30.5l-.2 20.8c2.8.5 5.6 1.2 8.5 1.8l3.3.8l-.2-23.4l-5.7.9zm-287.4 30.7c-16.5-.2-33.5 1.9-51.1 6.1l-2.86 18.8c23.26-3.3 75.96-6.9 127.56 14.6c4-1.6 8.2-3.1 12.4-4.3l1.2-8c-26.6-18.2-55.8-26.8-87.2-27.2m241.2 0c-31.4.4-60.6 9-87.2 27.2l1.2 8c4.2 1.2 8.4 2.7 12.4 4.3c51.6-21.5 104.3-17.9 127.6-14.6l-2.9-18.8c-17.6-4.2-34.6-6.3-51.1-6.1m-258.1 39c-17.91 0-32.1 1.8-39.69 3.1l-7.05 46.3l72.94 11.1c10.1-20.3 25.5-37.5 44.5-49.6c-25.4-8.5-50.4-10.9-70.7-10.9m275 0c-20.3 0-45.3 2.4-70.7 10.9c19 12.1 34.4 29.3 44.5 49.6l72.9-11.1l-7-46.3c-7.6-1.3-21.8-3.1-39.7-3.1m-137.5 10c-49.9 0-90.4 40.5-90.4 90.4S204.1 443 254 443s90.4-40.5 90.4-90.4s-40.5-90.4-90.4-90.4M64.27 315.5l1.36 85.5l-46.73 87h18.94l33.24-62l15.19 62h17.23l-21.19-86l-1.33-84zM433.6 317l-14.2 2.2l-.8 74.1l-24.2 55.7l7.4 25l24.7-57l30.9 71h18.2l-41.2-94.7zm-279.7 11.6c-4.7 12.1-7.2 25.2-7.2 38.9C146.7 427 194.8 475 254 475s107.3-48 107.3-107.5c0-13.7-2.5-26.8-7.2-38.9c1.8 7.7 2.8 15.8 2.8 24C356.9 409 310.8 456 254 456s-102.9-47-102.9-103.4c0-8.2 1-16.3 2.8-24m-18 77.4l-20.2 82h25.7l11.8-48c-7.4-11-13.3-22-17.3-34m236.2 0c-4 12-9.9 23-17.3 34l11.8 48h25.7z\"/>",
    "label": "Drums"
  },
  "perc": {
    "vb": "0 0 512 512",
    "body": "<path fill=\"currentColor\" d=\"M256 59.5a48 48 0 0 0-41.8 24.55c13.3-2.32 27.2-3.55 41.8-3.55s28.6 1.24 41.9 3.56A48 48 0 0 0 256 59.5m0 39c-99.4 0-163.7 59.3-186.6 132.3c-23 73.2-6.4 160.5 52.3 219v.1l2.6 2.6h3.7c17.8 0 32.9-7.3 50.5-14.7l-3.6-17.9c-15.4 6.1-27.4 11.3-38 13.5l-5.1 1.1l-3.5-3.9c-48.4-53.3-62-130.1-41.7-194.4c20.9-66.7 77-119.7 169.4-119.7c92.3 0 148.5 53 169.4 119.7c20.3 64.3 6.7 141.1-41.7 194.4l-3.5 3.9l-5.1-1.1c-10.6-2.2-22.6-7.4-38-13.5l-3.6 17.9c17.6 7.4 32.7 14.7 50.5 14.7h3.7l2.6-2.6v-.1c58.7-58.5 75.3-145.8 52.3-219c-22.9-73-87.2-132.3-186.6-132.3m-151 34a48 48 0 0 0-48 48a48 48 0 0 0 4.6 20.4c11.7-25.5 28.5-48.7 49.9-67.9a48 48 0 0 0-6.5-.5m302 0a48 48 0 0 0-6.5.5c21.4 19.2 38.2 42.4 49.9 67.9a48 48 0 0 0 4.6-20.4a48 48 0 0 0-48-48m-151 2c-13.3 0-25.7 1.1-37.2 3.2a48 48 0 0 0 37.2 17.8a48 48 0 0 0 37.2-17.7q-17.25-3.3-37.2-3.3m-104.7 33.7c-19.3 16-33.4 36.7-42.7 60.1a48 48 0 0 0 44.4-47.8a48 48 0 0 0-1.7-12.3m209.3 0a48 48 0 0 0-1.6 12.3a48 48 0 0 0 44.5 47.9c-9.4-23.5-23.5-44.2-42.9-60.2M95.7 299.4c.5 23.4 5.3 46.9 14.4 68.8a48 48 0 0 0 8.9-27.7a48 48 0 0 0-23.3-41.1m320.6 0a48 48 0 0 0-23.3 41.1a48 48 0 0 0 8.9 27.8c9.1-22 13.9-45.5 14.4-68.9m-374.8 3.3A48 48 0 0 0 23 340.5a48 48 0 0 0 35.9 46.4c-11-26.9-16.7-55.6-17.4-84.2m429.1 0c-.8 28.6-6.5 57.2-17.5 84.2a48 48 0 0 0 35.9-46.4a48 48 0 0 0-18.4-37.8M256 402.5c-25.6 0-46.5 5-64 11l3.5 17.7c16.8-5.9 36.2-10.7 60.5-10.7s43.7 4.8 60.5 10.7l3.5-17.7c-17.5-6-38.4-11-64-11\"/>",
    "label": "Hand drum"
  },
  "shaker": {
    "vb": "0 0 512 512",
    "body": "<path fill=\"currentColor\" d=\"M91.56 31.29c-5.95.08-12.02 1.34-18.35 4.04c-12.65 5.39-20.44 14.03-25.61 25.25c-5.17 11.21-7.36 25.08-7.39 39.42c-.1 28.8 8.69 59.4 14.38 72.7c6.83 16 18.02 32.2 31.05 46.5l111.66-47.6c-1.3-19.4-5.2-38.6-12.1-54.7c-5.6-13.3-21.7-40.8-42.4-60.63c-10.4-9.91-21.9-17.92-33.6-21.95c-5.8-2.02-11.69-3.13-17.64-3.03M420.7 107c-11 .1-23 2.8-34.8 7.2c-26.9 10.1-52.4 29.1-62.9 39.2c-12.5 12-23.7 28.2-32.5 45.5l84 87.6c17.7-8.1 34.3-18.5 46.9-30.6c10.4-10.1 30.5-34.8 41.7-61.2c5.6-13.2 9-26.8 8.7-39.2c-.4-12.3-4.2-23.3-13.7-33.3c-9.5-9.9-20.3-14.1-32.7-15c-1.5-.1-3.1-.2-4.7-.2m-222.9 84l-98.52 42c3.92 3.6 7.82 7 11.82 10.2l85.9-36.7c.5-5.1.8-10.3.8-15.5m84.7 25.5c-2 4.9-3.8 9.8-5.4 14.6l64.6 67.5c4.9-1.4 9.9-3 14.9-4.8zM194 227.4l-65.7 28c7.8 4.9 15.3 8.8 22 11.2c10.5 3.9 18.7 4.1 22.6 2.4s9.4-7.7 13.9-17.9c2.9-6.6 5.3-14.7 7.2-23.7m77.7 24.1c-1.8 9-2.8 17.4-2.7 24.6c.1 11.2 2.8 18.9 5.8 22c2.9 3 10.5 6.1 21.7 6.6c7.1.4 15.6-.2 24.6-1.6zm-75.6 20c-4.3 6.3-9.6 11.3-16.2 14.1s-13.8 3.2-21.3 1.9c8.1 15.6 16.8 32.1 25.6 50.4c9.2-9.1 17.6-18 25.6-26.6c-5-14-9.4-27.2-13.7-39.8m56.3 19.7c-33.1 36.9-70.6 80.5-138.9 129.7c-14.13 13.7-11.2 26.7-3.3 35c7.9 8.2 20.8 11.6 35.1-1.9c52-66.2 97.2-101.8 135.5-133.3c-7.5-1.8-14-5-19-10.2s-7.9-11.8-9.4-19.3m-14.2 89.3c-8.4 7.6-16.9 15.7-25.7 24.5c6.3 17.6 12.5 36.8 18.2 58.2c7.9 18.1 21.2 20 31.7 15.5s18.2-15.3 10.6-33.5c-14.1-23.2-25.4-44.7-34.8-64.7\"/>",
    "label": "Shaker"
  },
  "mallet": {
    "vb": "0 0 512 512",
    "body": "<path fill=\"currentColor\" d=\"M25 57v270h78V57zm96 16v238h78V73zm-57 7a16 16 0 0 1 16 16a16 16 0 0 1-16 16a16 16 0 0 1-16-16a16 16 0 0 1 16-16m153 9v206h78V89zm-57 7a16 16 0 0 1 16 16a16 16 0 0 1-16 16a16 16 0 0 1-16-16a16 16 0 0 1 16-16m153 9v174h78V105zm-57 7a16 16 0 0 1 16 16a16 16 0 0 1-16 16a16 16 0 0 1-16-16a16 16 0 0 1 16-16m153 9v142h78V121zm-57 7a16 16 0 0 1 16 16a16 16 0 0 1-16 16a16 16 0 0 1-16-16a16 16 0 0 1 16-16m96 16a16 16 0 0 1 16 16a16 16 0 0 1-16 16a16 16 0 0 1-16-16a16 16 0 0 1 16-16m0 64a16 16 0 0 1 16 16a16 16 0 0 1-16 16a16 16 0 0 1-16-16a16 16 0 0 1 16-16m-96 16a16 16 0 0 1 16 16a16 16 0 0 1-16 16a16 16 0 0 1-16-16a16 16 0 0 1 16-16m-96 16a16 16 0 0 1 16 16a16 16 0 0 1-16 16a16 16 0 0 1-16-16a16 16 0 0 1 16-16m-96 16a16 16 0 0 1 16 16a16 16 0 0 1-16 16a16 16 0 0 1-16-16a16 16 0 0 1 16-16m-96 16a16 16 0 0 1 16 16a16 16 0 0 1-16 16a16 16 0 0 1-16-16a16 16 0 0 1 16-16m352 57c-12.81 0-23 10.19-23 23s10.19 23 23 23s23-10.19 23-23s-10.19-23-23-23m-40.977 23.967l-162.242 40.56L65.117 375.07l-2.234 17.86l101.53 12.691L29.815 439.27l4.368 17.46l179.7-44.925l193.313 24.164a40.6 40.6 0 0 1 2.246-17.857l-147.187-18.4l117.162-29.29a40.6 40.6 0 0 1-4.395-17.455zM448 409c-12.81 0-23 10.19-23 23s10.19 23 23 23s23-10.19 23-23s-10.19-23-23-23\"/>",
    "label": "Marimba"
  },
  "eguitar": {
    "vb": "0 0 24 24",
    "body": "<path fill=\"currentColor\" d=\"M19.59 3H22v2h-1.59l-5.29 5.29l-1.41-1.39zM12 9c.26 0 .5.1.71.3l2 2c.18.2.29.43.29.7l-.1.4l-4 8c-.19.35-.54.53-.9.53c-.35 0-.71-.18-.89-.53l-1.86-3.7l-3.7-1.8c-.37-.2-.55-.55-.55-.9s.18-.7.55-.9l8-4c.14-.1.29-.1.45-.1m-2.65 2.82l-.7.68l2.85 2.85l.68-.7zm-1.41 1.41l-.71.71l2.83 2.83l.71-.71z\"/>",
    "label": "Electric guitar"
  },
  "aguitar": {
    "vb": "0 0 24 24",
    "body": "<path fill=\"currentColor\" d=\"M19.59 3H22v2h-1.59l-4.24 4.24c-.37-.56-.85-1.04-1.41-1.41zM12 8a4 4 0 0 1 4 4a3.99 3.99 0 0 1-3 3.87V16a5 5 0 0 1-5 5a5 5 0 0 1-5-5a5 5 0 0 1 5-5h.13c.45-1.76 2.04-3 3.87-3m0 2.5a1.5 1.5 0 0 0-1.5 1.5a1.5 1.5 0 0 0 1.5 1.5a1.5 1.5 0 0 0 1.5-1.5a1.5 1.5 0 0 0-1.5-1.5m-5.06 3.74l-.71.7l2.83 2.83l.71-.71z\"/>",
    "label": "Acoustic guitar"
  },
  "piano": {
    "vb": "0 0 24 24",
    "body": "<path fill=\"currentColor\" d=\"M20 2H4c-1.1 0-2 .9-2 2v16a2 2 0 0 0 2 2h16c1.11 0 2-.89 2-2V4a2 2 0 0 0-2-2m-5.26 12H15v6H9v-6h.31c.55 0 .99-.44.99-1V4h3.45v9c0 .56.44 1 .99 1M4 4h2.8v9c0 .56.44 1 .99 1H8v6H4zm16 16h-4v-6h.26c.55 0 .99-.44.99-1V4H20z\"/>",
    "label": "Piano"
  },
  "synth": {
    "vb": "0 0 24 24",
    "body": "<path fill=\"currentColor\" d=\"M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10s10-4.5 10-10S17.5 2 12 2m1 8h-2V4.1c.3-.1.7-.1 1-.1s.7 0 1 .1z\"/>",
    "label": "Synth"
  },
  "violin": {
    "vb": "0 0 24 24",
    "body": "<path fill=\"currentColor\" d=\"M11 2a1 1 0 0 0-1 1v6a.5.5 0 0 0 .5.5H12a.5.5 0 0 1 .5.5a.5.5 0 0 1-.5.5h-1.5C9.73 10.5 9 9.77 9 9V5.16C7.27 5.6 6 7.13 6 9v1.5A2.5 2.5 0 0 1 8.5 13A2.5 2.5 0 0 1 6 15.5V17c0 2.77 2.23 5 5 5h2c2.77 0 5-2.23 5-5v-1.5a2.5 2.5 0 0 1-2.5-2.5a2.5 2.5 0 0 1 2.5-2.5V9c0-2.22-1.78-4-4-4V3a1 1 0 0 0-1-1zm-.25 14.5h2.5l-.5 3.5h-1.5z\"/>",
    "label": "Strings"
  },
  "harp": {
    "vb": "0 0 512 512",
    "body": "<path fill=\"currentColor\" d=\"m120.7 27.53l-28.93 8.56C112.1 187.8 125.6 321.9 183.9 455H228c-40-140.6-84.2-280.4-107.3-427.47m19.9 10.36c2.6 16.05 5.5 32.03 8.7 47.95c9.8 2.67 19.9 6.9 30.1 11.85c18.3 8.71 37.3 19.81 56.1 29.51s37.3 17.9 53.6 21.1c16.4 3.2 29.8 1.7 41.6-7c22.2-16.4 38.4-26.2 51.3-31.4c12.8-5.2 23.3-5.9 31.6-1.8c7.8 4 11.5 11.1 14.2 17.1l5.1-11.3c-7.3-12.9-18.1-21.38-32.9-23.26c-16.2-2.05-38.4 4.06-66 25.66c-14.4 11.3-33 9.7-50.6 2.8c-17.5-6.9-35.7-18.9-54.2-31.64c-18.6-12.74-37.3-26.21-54.8-35.98c-12.3-6.86-23.8-11.66-33.8-13.59m12.8 68.31q7.2 34.5 15.6 68.7v-62.2c-5.4-2.6-10.6-4.8-15.6-6.5M404.8 124c-27 110.6-55.1 223.8-97.7 331h38.6c34.5-94.4 51-203.5 70.3-311.3c-1.8-3.8-3-7.4-4.3-10.4c-2.3-5.3-4.3-8.1-6.3-9.1c-.2-.1-.4-.2-.6-.2M199 128.1v160.4c5.9 21.1 11.9 42.1 18 63.1V137.8c-6.1-3.2-12.1-6.5-18-9.7M361 142q-8.1 5.4-18 12.6V288h1c5.9-20.9 11.5-41.8 17-62.8zm-114 10.8V455h18V160.2c-6-2.2-12-4.7-18-7.4m66 14.3c-5.9.7-11.9.8-18 .2v269.8c6.3-16.4 12.3-32.9 18-49.6zM163.9 473l-15.1 16h214.4l-15.1-16z\"/>",
    "label": "Harp"
  },
  "trumpet": {
    "vb": "0 0 24 24",
    "body": "<path fill=\"currentColor\" d=\"M22 6c-1 5-7 5-7 5H4c-1 0-2-1-2-1H1v4h1s1-1 2-1h.3c-.2.3-.3.6-.3 1v2c0 1.1.9 2 2 2h1v1h2v-1h1v1h2v-1h1v1h2v-1h1c1.1 0 2-.9 2-2v-2c0-.1 0-.3-.1-.4c1.7.6 3.5 1.8 4.1 4.4h1V6zM6 16.5c-.3 0-.5-.2-.5-.5v-2c0-.3.2-.5.5-.5h1v3zm3 0v-3h1v3zm3 0v-3h1v3zm4.5-.5c0 .3-.2.5-.5.5h-1v-3h1c.3 0 .5.2.5.5zM9 10H7V9h2zm3 0h-2V9h2zm3 0h-2V9h2z\"/>",
    "label": "Brass"
  },
  "sax": {
    "vb": "0 0 24 24",
    "body": "<path fill=\"currentColor\" d=\"M4 2a1 1 0 0 0-1 1a1 1 0 0 0 1 1a3 3 0 0 1 3 3v8.5c0 3.6 2.9 6.5 6.5 6.5s6.5-2.9 6.5-6.5V13a1 1 0 0 0 1-1a1 1 0 0 0-1-1h-6a1 1 0 0 0-1 1a1 1 0 0 0 1 1v2a1 1 0 0 1-1 1a1 1 0 0 1-1-1v-4a1 1 0 0 0 1-1a1 1 0 0 0-1-1V8a1 1 0 0 0 1-1a1 1 0 0 0-1-1v-.5A3.5 3.5 0 0 0 8.5 2z\"/>",
    "label": "Sax"
  },
  "flute": {
    "vb": "0 0 512 512",
    "body": "<path fill=\"currentColor\" d=\"M449.4 26.29c-5.4 2.56-14.6 7-27.2 14.14c-17.6 9.98-37.7 23.64-45.9 34.27c-1.8 2.33-5 10.39-6.8 17.25c-1.2 4.49-1.4 6.02-1.8 8.25l44 44c2.2-.4 3.8-.6 8.3-1.8c6.9-1.8 15-5 17.3-6.8c10.6-8.2 24.3-28.2 34.2-45.79c7.2-12.62 11.6-21.81 14.2-27.24zM435 54.32l22.7 22.63l-12.8 12.74l-22.7-22.63zm-77.1 61.48L42.49 431.3c7.86 3 15.66 8.3 22.77 15.4c7.09 7.1 12.37 14.9 15.42 22.7L396.1 154zm-8.3 30.7h.1a16 16 0 0 1 11.1 4.7a16 16 0 0 1 0 22.6a16 16 0 0 1-22.7 0a16 16 0 0 1 0-22.6a16 16 0 0 1 11.5-4.7m-45.1 45.3a16 16 0 0 1 11 4.6a16 16 0 0 1 0 22.7a16 16 0 0 1-22.6 0a16 16 0 0 1 0-22.7a16 16 0 0 1 11.6-4.6M259.2 237a16 16 0 0 1 11.1 4.7a16 16 0 0 1 0 22.6a16 16 0 0 1-22.6 0a16 16 0 0 1 0-22.6a16 16 0 0 1 11.5-4.7m-45.3 45.3h.1a16 16 0 0 1 11 4.7a16 16 0 0 1 0 22.6a16 16 0 0 1-22.6 0a16 16 0 0 1 0-22.6a16 16 0 0 1 11.5-4.7m-45.2 45.3a16 16 0 0 1 11.1 4.6a16 16 0 0 1 0 22.7a16 16 0 0 1-22.7 0a16 16 0 0 1 0-22.7a16 16 0 0 1 11.6-4.6M29.04 446.5c-1.44 0-2.13.4-2.25.5c-.21.2-1.2 2.3.43 7.5c1.7 5.2 5.87 12.1 12.02 18.2c6.15 6.2 13.01 10.4 18.24 12.1c5.16 1.6 7.29.6 7.5.4s1.2-2.4-.43-7.5c-1.69-5.2-5.87-12.1-12.02-18.3c-6.15-6.1-13.01-10.3-18.24-12c-1.93-.6-3.44-.8-4.58-.9z\"/>",
    "label": "Flute"
  },
  "mic": {
    "vb": "0 0 24 24",
    "body": "<path fill=\"currentColor\" d=\"M12 2a3 3 0 0 1 3 3v6a3 3 0 0 1-3 3a3 3 0 0 1-3-3V5a3 3 0 0 1 3-3m7 9c0 3.53-2.61 6.44-6 6.93V21h-2v-3.07c-3.39-.49-6-3.4-6-6.93h2a5 5 0 0 0 5 5a5 5 0 0 0 5-5z\"/>",
    "label": "Mic"
  },
  "voice": {
    "vb": "0 0 24 24",
    "body": "<path fill=\"currentColor\" d=\"M9 5a4 4 0 0 1 4 4a4 4 0 0 1-4 4a4 4 0 0 1-4-4a4 4 0 0 1 4-4m0 10c2.67 0 8 1.34 8 4v2H1v-2c0-2.66 5.33-4 8-4m7.76-9.64c2.02 2.2 2.02 5.25 0 7.27l-1.68-1.69c.84-1.18.84-2.71 0-3.89zM20.07 2c3.93 4.05 3.9 10.11 0 14l-1.63-1.63c2.77-3.18 2.77-7.72 0-10.74z\"/>",
    "label": "Vocals"
  },
  "disc": {
    "vb": "0 0 24 24",
    "body": "<path fill=\"currentColor\" d=\"M12 11a1 1 0 0 0-1 1a1 1 0 0 0 1 1a1 1 0 0 0 1-1a1 1 0 0 0-1-1m0 5.5c-2.5 0-4.5-2-4.5-4.5s2-4.5 4.5-4.5s4.5 2 4.5 4.5s-2 4.5-4.5 4.5M12 2A10 10 0 0 0 2 12a10 10 0 0 0 10 10a10 10 0 0 0 10-10A10 10 0 0 0 12 2\"/>",
    "label": "Vinyl"
  }
};

export const ICON_KEYS = Object.keys(INSTRUMENT_ICONS);

/** 内联渲染一个乐器图标(currentColor 跟随父级 color)。未知 key 回退到默认 wav。 */
export function InstrumentIcon({ icon, size = 16, style }: { icon?: string | null; size?: number; style?: CSSProperties }) {
  const def = INSTRUMENT_ICONS[icon || DEFAULT_ICON] ?? INSTRUMENT_ICONS[DEFAULT_ICON];
  return <svg width={size} height={size} viewBox={def.vb} aria-hidden="true" style={style} dangerouslySetInnerHTML={{ __html: def.body }} />;
}
