# hhgen — 本地拆轨/切 loop 库(v0,已搁置)

> ⚠️ **这是 SunoGrid 的 v0 前身,目前已搁置。** 当前产品改用 Suno 在线生成 loop(见仓库根 `README.md` 与 `PRODUCT.md`)。
> 本目录保留作历史与备用:可能仅在"导入外部样本估 BPM / 离线分析"时复用。

把整曲拆成分轨、再按小节切成带元数据的 loop 库,供下游"自动做 beat"工具直接消费。
...
- v0 没做时间拉伸，存的是真实 BPM，对齐留给下游生成工具。
