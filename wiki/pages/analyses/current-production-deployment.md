...
生产机为 `cable00`,仓库根目录是 `/data/deploy/sunogrid`,web 服务由 supervisord 的 `sunogrid-web` 管理。以后常规上线在仓库根目录执行 `./release.sh`;脚本会对齐 `origin/main`,构建、迁移、重启 `sunogrid-web`,再检查 `http://127.0.0.1:3000/api/health`。`sunogrid-stem` 是独立服务,常规 web 上线不重启。
