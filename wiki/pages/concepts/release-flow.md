...
- `release.sh` 默认参数:
  ...
  - `DEFAULT_WEB_PORT=3037`
  - `PORT` 默认为空;显式设置时优先于默认端口。
  - `RUN_DB_MIGRATIONS=1`
  - `RUN_HEALTHCHECK=1`
- 更新逻辑:
...
- 当前线上 web 端口是 `3037`。
- 健康检查不猜测端口:优先使用 `HEALTHCHECK_URL`,其次使用显式 `PORT`,再其次读取 supervisord 进程实际 `PORT`;读不到时使用项目记录的当前线上默认端口 `3037`。
